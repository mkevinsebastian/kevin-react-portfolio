      ...
      <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/20 before:to-transparent">
        {workExperience.map((job, index) => (
          <div className={cardClass} key={index}>
             <div className="absolute left-1/2 w-4 h-4 rounded-full bg-brand-cyan border-4 border-navy -translate-x-1/2 -translate-y-1/2 top-0 z-10 hidden md:block"></div>
            ...
            {job.techStack && (
              <div className="flex flex-wrap gap-2 mt-4">
                {job.techStack.map((tech, i) => (
                  <span key={i} className="text-xs font-medium bg-white/10 text-white px-3 py-1 rounded-full border border-white/20 hover:bg-brand-cyan/20 hover:border-brand-cyan transition-colors">
                    {tech}
                  </span>
                ))}
              </div>
